...
<xsl:value-of select="$access"/> class <xsl:value-of select="$class"/>
: MfGames.Utility.Settings
{
    #region Constructors
    public <xsl:value-of select="$class"/>(ISettings settings)
    : base(settings)
    {
    }
    #endregion
...
<xsl:text>;
  ...
  <xsl:template name="string-to-variable">
	...
	<xsl:value-of select="$value"/>
      ...
      <xsl:when test="muc:type/@format = 'enum'">
	<xsl:value-of select="$value"/>
      </xsl:when>
      <xsl:otherwise>
  ...
  <xsl:template name="variable-to-variable">
	...
	<xsl:text>(</xsl:text>
	<xsl:value-of select="muc:type"/>
	<xsl:text>) Enum.Parse(typeof(</xsl:text>
	<xsl:value-of select="muc:type"/>
	<xsl:text>), </xsl:text>
	<xsl:value-of select="$value"/>
	<xsl:text>)</xsl:text>
